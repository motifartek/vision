...
```powershell
# Windows
.\apps\ai\sonic\scripts\fetch-models.ps1             # varsayılan: ced-base
.\apps\ai\sonic\scripts\fetch-models.ps1 -Model ced-tiny  # hızlı ama daha çok yanlış pozitif
```
...
```bash
# CPU (varsayılan)
cargo build -p sonic --release

# GPU seçenekleri
cargo build -p sonic --release --features directml   # Windows, her DX12 GPU
cargo build -p sonic --release --features cuda        # CUDA 12 + cuDNN 9
cargo build -p sonic --release --features tensorrt    # fp16 motoru kendisi kurar
```
...
```bash
cargo run -p sonic --release --bin verify-mel
```
...
> [!IMPORTANT]
> **Gateway şu an istek yolunda değil.** Dashboard `/api/sonic` tüneliyle
> doğrudan sonic servisine bağlanıyor, video da Next.js'in statik klasöründen
...
cargo run -p sonic --release --bin compare-decoders -- apps/dashboard/public/media/test3.mp4
...
```bash
cargo build -p sonic --release --features directml   # Windows
cargo build -p sonic --release --features cuda        # Linux + CUDA 12
```
`/healthz` uç noktasında `providers` alanını kontrol edin.

### verify-mel kapısı geçilemiyor

Mel parametreleri bozulmuş. `apps/ai/sonic/src/audio/mel.rs` dosyasındaki sabitlerin şu değerlerde olduğunu kontrol edin:
